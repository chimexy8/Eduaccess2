    ...
    public class AccessController : ControllerBase
    {
        ...
        public AccessController(
            ...
            SignInManager<ApplicationUser> signInManager,
             IEmailSender emailSender, ApplicationDbContext context, IOptions<ApiDetail> options, IFlashHttpClient flashMoneyHttpClient, RoleManager<IdentityRole> roleManager)
        {
        ...
        public async Task<IActionResult> RegisterUser([FromBody] RegisterViewModel value)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    _logger.LogInformation("{user} was successfully Updated", value.Email);
                    var userexist = _context.User.Any(p => p.Email == email);
                    if (!userexist)
                    {
                        var user = new ApplicationUser { UserName = value.Email, PhoneNumber = value.Phone, RegisterationDate=value.Date };
                        var result = await _userManager.CreateAsync(user, value.Password);
                        var roleExist = await _roleManager.RoleExistsAsync(value.Role);
                        if (roleExist)
                        {
                        ...
                        {
                            await _roleManager.CreateAsync(new IdentityRole(value.Role));
                            await _userManager.AddToRoleAsync(user, value.Role);
                        }
                        _logger.LogInformation("Account created for {user}", value.Email);
                        ...
                        var infoo = new { Id = NewMember.Id, Email = NewMember.Email, FirstName = NewMember.FirstName, LastName = NewMember.LastName, Phone = NewMember.Phone, Token = token, DOB = dateofB };
                        return Ok(infoo);
        ...
        public async Task<IActionResult> RegisterSponsor([FromBody] RegisterViewModel value)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (!userexist)
                    {
                        ...
                        await _context.SaveChangesAsync();
                        var token = GenerateJWTToken();
                        var infoo = new { Id = NewMember.Id, Email = NewMember.Email, FirstName = NewMember.FirstName, LastName = NewMember.LastName, Phone = NewMember.Phone, Token = token};
                        return Ok(infoo);
        ...
        public async Task<IActionResult> Login([FromBody] LoginViewModel value)
        {
            ...
            if (user != null && await _userManager.CheckPasswordAsync(user, value.Password))
            {
                ...
                var info = new { Id = userr.Id, Email = userr.Email, FirstName = userr.FirstName, LastName = userr.LastName, Phone = userr.Phone, Token = token, Role=userr.Role, Passport = userr.Passport };
                return Ok(info);
            }
