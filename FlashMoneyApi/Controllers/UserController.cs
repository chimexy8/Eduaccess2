    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(ApplicationDbContext context, IEmailSender emailSender, ILogger<UserController> logger,
            UserManager<ApplicationUser> userManager, IOptions<ApiDetail> options,
             IFlashHttpClient flashMoneyHttpClient, RoleManager<IdentityRole> roleManager)
        {
            _context = context;
        ...
        public async Task<IActionResult> ResendCode([FromBody] ResendDTO value)
        {
            async Task sendOPT(string OTP, string phonee)
            {
                ...
                await _emailSender.SendEmailAsync(phonee, "Confirm Email", OTP);
            }
        ...
        public async Task<IActionResult> Phone([FromBody]BvnOrPhoneModel value)
        {
            ...
            if (!string.IsNullOrEmpty(value.Email))
            {
                ...
                var user = _context.User.SingleOrDefault(p => p.Email == value.Email);
                if (user==null)
                {
                    if (confirmawaitinglist!=null)
                    {
                        try
                        {
                            ...
                            confirmawaitinglist.Role = value.Role;
                            ...
                            await _emailSender.SendEmailAsync(Email, "Verification Code", $"{hjh} Is your verification Code, it will expire in 10 minutes.");
                    ...
                    else
                    {
                        try
                        {
                            var newconfirmawaitinglist = new ConfirmationAwaiting { Active = true, Code = hjh, Email = Email, Time = DateTime.Now , Role=value.Role};
        ...
        public async Task<IActionResult> ValidatePhone([FromBody] OtpDTO value)
        {
            ...
            else
            {
                ...
                //_context.SaveChanges();
                return Ok(new UserCheckInfo { Status = "success", ReasonPhrase = "New User", Email = valid.Email, Role=valid.Role});
            }
        ...
        //This API method returns the userRoles of the User
        [HttpGet("GetUserRole/{email}")]
        public async Task<IActionResult> GetUserRole(string email)
        {
            var userRole = _context.User.FirstOrDefault(x => x.Email == email);
            if(userRole != null)
            {
                UserCheckInfo checks = new UserCheckInfo();
                checks.Role = userRole.Role;
