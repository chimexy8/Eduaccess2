...
namespace FlashMoney.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> Register(AuthenticationModel AuthenticationModel)
        {
            ...
            {
                if (!string.IsNullOrEmpty(AuthenticationModel.BvnOrPhoneModel?.Email))
                {

                    try
                    {
                        var addr = new System.Net.Mail.MailAddress(AuthenticationModel.BvnOrPhoneModel.Email);
                    ...
                    using (var client = _flashMoneyHttpClient.GetClient())
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                var gjh = new OTPValidationModel { Phone = AuthenticationModel.BvnOrPhoneModel.Email };
            ...
            if (ModelState.IsValid)
            {
                using (var client = _flashMoneyHttpClient.GetClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            var g = new SignUpViewmodel { Email = result.Email, Role = result.Role };
                            //return RedirectToAction("SignUp", "Auth", result);
                            if (g.Role == "sponsor")
                            {
                                return PartialView("_SponsorSignUp", g);
                            }
        ...
        public async Task<IActionResult> SignUp(SignUpViewmodel signUpViewmodel)
        {
            ...
            {
                DateTime dateofB;
                signUpViewmodel.Date = DateTime.Now;
                var d = DateTime.TryParseExact(signUpViewmodel.date, "dd/MM/yyyy", null, DateTimeStyles.None, out dateofB);
            ...
            if (signUpViewmodel.Role=="sponsor")
            {
                return PartialView("_SponsorSignUp", signUpViewmodel);
            }
            return PartialView("_SignUp", signUpViewmodel);
        }
         [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SponsorSignUp(SponsorSignUpViewModel signUpViewmodel)
        {
            if (ModelState.IsValid)
            {
                // DateTime dateofB;
                ...
                using (var client = _flashMoneyHttpClient.GetClient())
                {
                    ...
                    var json = JsonConvert.SerializeObject(signUpViewmodel);
                    var content = new StringContent(json, Encoding.UTF8, "application/json");
                    var response = await client.PostAsync("CreateSponsor", content);
                    if (response.IsSuccessStatusCode)
                    {
                        var b = await response.Content.ReadAsStringAsync();
                        var gg = JsonConvert.DeserializeObject<LoginDTO>(b);
                        ...
                        if (gg != null)
                        {
                            ...
                            //return RedirectToAction("RegistrationSuccess", new { pin = gg.Pin });
                            PinDTO g;
                            if (signUpViewmodel.Role=="student")
                            {
                               g = new PinDTO { Pin = gg.Pin, Role=1 };
                            }
                            else
                            {
                                 g = new PinDTO { Pin = gg.Pin, Role=2};
                            }
        ...
        public async Task<IActionResult> SignIn(AuthenticationModel AuthenticationModel, string returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                using (var client = _flashMoneyHttpClient.GetClient())
                {
                    ...
                    {
                        await _NotificationHub.Clients.User($"{AuthenticationModel.SignInViewModel.Phone}").SendAsync("Notify", "NewUser");
